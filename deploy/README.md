...
Для уведомлений об отсутствии доступных прокси на сервере нужен `curl`. В `/etc/proxy-microservice.env` можно задать `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` (числовой ID чата либо `@channel_username`); обе переменные заполняются вместе. Без них уведомления отключены. После изменения настроек перезапустить сервис. Отправка идёт в фоне: не более одной попытки в 60 секунд на пользователя и не более двух одновременно на процесс. Повторы в этот период и запросы при занятых слотах не ставятся в очередь; ошибка Telegram не меняет ответ API. Токен, данные прокси и поисковые запросы в логи не попадают.
