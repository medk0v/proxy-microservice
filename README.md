...
Нужны Rust 1.88+ и общий локальный контейнер PostgreSQL `postgres`, доступный на порту `5432`. Для запуска приложения в Docker нужен Docker Compose. Отдельный контейнер БД для этого проекта не создаётся.

При первой настройке создайте `.env`, отдельную роль и базу внутри общего PostgreSQL:

```bash
python3 scripts/setup.py
docker start postgres  # если общий контейнер остановлен
docker exec -it postgres createuser -U postgres --createdb --pwprompt proxy_service
docker exec postgres createdb -U postgres --owner=proxy_service proxy_service
```

На запрос `createuser` введите значение `POSTGRES_PASSWORD` из `.env`. Права `CREATEDB` нужны для интеграционных тестов. Если роль и база уже созданы или перенесены, повторять эти команды не нужно.
...
Откройте **http://localhost:8080**. Логин — `ADMIN_USERNAME`, пароль — `ADMIN_PASSWORD` в `.env`. Скрипт создаёт случайные пароли и ключ шифрования; существующий `.env` не перезаписывает. Сервис подключается к `127.0.0.1:5432/proxy_service`, данные хранятся в томе общего PostgreSQL. Миграции применяются при запуске сервиса.

Запуск приложения через Docker после настройки общей БД:

```bash
docker compose --profile full up --build -d
```

Приложение в Docker подключается к общей БД через `host.docker.internal:5432`. Остановка приложения: `docker compose --profile full down`; общий PostgreSQL и его данные остаются на месте. Если общий PostgreSQL опубликован на другом порту, измените `POSTGRES_PORT` и порт в `DATABASE_URL` одновременно. Если занят порт приложения, согласованно измените `BIND_ADDR`, `APP_PORT` и `APP_ORIGIN`.
...
Отправка идёт в фоне и не меняет ответ API: `/random` возвращает `404`, список/экспорт — пустой результат. Для всех пользователей, операций и фильтров действует общий лимит: не более одной попытки отправки в час. Лимит хранится в памяти и сбрасывается при перезапуске сервиса. Ошибка Telegram не блокирует выдачу; она фиксируется в логе без секретов. Уведомления не сохраняются в очередь для повторной доставки.
...
- `APP_ORIGIN` должен точно совпадать с адресом в браузере, без завершающего `/`. Для внешнего домена нужны HTTPS и `COOKIE_SECURE=true`; HTTP допускается только для localhost. Compose публикует приложение на loopback; публикация порта общей БД настраивается в её контейнере. TLS завершается на вашем reverse proxy.
